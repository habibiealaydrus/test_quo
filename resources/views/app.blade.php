...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('build/assets/app-CcclaFIk.css') }}">
    <script src="{{ asset('build/assets/app-BaRvFqc9.js') }}"></script>
...
<body>
    ...
    function getNotification() {
        ...
        el.html(
            '<li class="list-group-item list-group-item-action text-center">😇 Empty Notification</li>'
        );
        const requestUrl = '{{ url("/notifications") }}'; // Cukup URL dasar ini
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.notification && Array.isArray(res.notification) && res.notification.length > 0) {
                    el.html('');
                    $('.countNotification').html(res.notification.length);

                    $.each(res.notification, function (key, val) {
                        const follupUrl = val.follup_url || '#';
                        const title = val.title || 'Untitled Notification';
                        const content = val.content || 'No content provided.';
                        const createdFormat = val.created_format || 'Unknown date';

                        const listItem = $('<li class="list-group-item list-group-item-action dropdown-notifications-item"></li>');
                        const notificationHtmlContent = `
                            <a href="${follupUrl}" class="d-flex">
                                ...
                                <div class="flex-grow-1">
                                    ...
                                    <small class="text-muted">${createdFormat}</small>
                                </div>
                            </a>
                        `;
                        listItem.html(notificationHtmlContent);
                        el.append(listItem);
                    });
                } else {
                    console.log('No notifications to display or array is empty/invalid.');
